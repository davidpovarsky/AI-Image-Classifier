...
TUF supplies root rotation, timestamp/snapshot/targets metadata, consistent snapshots, expiry, target size/hash verification, and rollback protection. Bundle signatures remain defense in depth and support offline import. The checked-in root is a self-signed development trust root whose key ID is rejected by the public-release gate. `product/scripts/generate_internal_tuf_repository.py` creates a complete private CI repository for vendor, tenant, and device targets; production replaces only the public root and remote signer configuration.
